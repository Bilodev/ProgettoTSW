...
public class OrdineRiepilogoDAO {
    ...
    // Costante SQL comune per evitare la duplicazione delle lunghe clausole di SELECT e JOIN delle testate
    private static final String BASE_SELECT_RIEPILOGO = 
        "SELECT o.id AS ordine_id, o.seqId, o.data_ordine, " +
        "SUM(od.prezzo_unitario * od.quantita) AS totale, " +
        "u.username, u.email, " +
        ...
        "JOIN ORDINE_DVD od ON o.id = od.ordine_id " +
        "JOIN DVD d ON d.id = od.DVD_id ";

    private static final String BASE_GROUP_BY = 
        " GROUP BY o.id, o.seqId, o.data_ordine, u.username, u.email, " +
        "i.nome, i.cognome, i.indirizzo, i.citta, i.cap, i.paese ";

    // Ordini di un utente specifico
    public List<OrdineRiepilogo> findByUtente(int utenteId) throws SQLException {
        String sql = BASE_SELECT_RIEPILOGO + " WHERE u.email = ? " + BASE_GROUP_BY + " ORDER BY o.data_ordine DESC";
    ...
    public List<OrdineRiepilogo> findByDateRange(String dataInizio, String dataFine) throws SQLException {
        String sql = BASE_SELECT_RIEPILOGO + " WHERE o.data_ordine BETWEEN ? AND ? " + BASE_GROUP_BY + " ORDER BY o.data_ordine DESC";
        List<OrdineRiepilogo> results = new ArrayList<>();
    ...
    public List<OrdineRiepilogo> findByUtenteAndDateRange(String utenteId, String dataInizio, String dataFine) throws SQLException {
        String sql = BASE_SELECT_RIEPILOGO + " WHERE u.email = ? AND o.data_ordine BETWEEN ? AND ? " + BASE_GROUP_BY + " ORDER BY o.data_ordine DESC";
        List<OrdineRiepilogo> results = new ArrayList<>();
    ...
    public List<OrdineRiepilogo> findTotaliPerCliente(String dataInizio, String dataFine) throws SQLException {
        String sql =
            ...
            "COUNT(DISTINCT o.id) AS num_ordini, " + // Conta gli ordini univoci sulla tabella principale
            "SUM(od.prezzo_unitario * od.quantita) AS totale_speso " +
            "FROM UTENTE u " +
